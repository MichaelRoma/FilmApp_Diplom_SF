//
//  RoundingTransiction.swift
//  FilmApp
//
//  Created by Mykhailo Romanovskyi on 25.09.2021.
//

import Foundation
import UIKit
class RoundingTransiction: NSObject {
    var round = UIView()
    ...
    var time = 0.3
    enum RoundingTransictionProfile: Int {
        case show, cancel, pop
    }
    var transitionProfile: RoundingTransictionProfile = .show
}

extension RoundingTransiction: UIViewControllerAnimatedTransitioning {
    ...
    func roundFrame(withViewCenter viewCenter: CGPoint, size viewSize: CGSize, startPoint: CGPoint) ->CGRect {
        ...
        let offsetVektor = sqrt(xLength * xLength + yLength * yLength)
        let size = CGSize(width: offsetVektor, height: offsetVektor)
